<script lang="ts">
  ...
  const { items = [], showSlideNumbers = true }: { items: string[]; showSlideNumbers: boolean } =
    $props();
...
<Carousel.Root
        ...
        <Card.Root class="border-border bg-popover flex h-full items-center justify-center p-0">
          <Card.Content class="flex items-center justify-center rounded-lg p-4">
            ...
            {:else if getMediaType(item) === "video"}
              <video
                class="pointer-events-none mx-auto max-h-96"
                autoPlay={true}
                muted={true}
                loop={true}>
  ...
  {#if showSlideNumbers}
    <div
      class="py-2 text-center text-sm text-neutral-600 transition-colors duration-300 hover:text-neutral-400">
